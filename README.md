...
Also published on [GitHub](https://github.com/escline/InstallCert).
...
```
brew tap andersonlfeitosa/java-install-cert
```

>**Note** since Java 11, you can run it directly without compiling it first:

```
brew install andersonlfeitosa/java-install-cert
```
...
```
java-install-cert [--proxy=proxyHost:proxyPort] <host>[:port] [passphrase]
```
...
```
java-install-cert woot.com:443
